...
namespace Ferrex
{
	...
	public class Gizmo_ToggleCrafting : Command
	{
		...
		public Gizmo_ToggleCrafting(IPawnCrafter crafter)
		{
			...
			{
				defaultLabel = Translator.Translate(labelStart);
				defaultDesc = Translator.Translate(descriptionStart);
				icon = Gizmo_ToggleCrafting.startIcon;
				return;
			}
			if (crafter.PawnCrafterStatus() == CrafterStatus.Crafting || crafter.PawnCrafterStatus() == CrafterStatus.Filling)
			{
				defaultLabel = Translator.Translate(labelStop);
				defaultDesc = Translator.Translate(descriptionStop);
				icon = Gizmo_ToggleCrafting.stopIcon;
			}
		}
		...
		public static Texture2D startIcon = ContentFinder<Texture2D>.Get("UI/Commands/Trade", true);
